...
# ── TOTAIS ────────────────────────────────────────────────────────────────────
# ultima_dt calculada ANTES de rf_rent para limitar o CDI à data correta
ultima_dt = min(f["dt_fim"] for f in fundos if f["cnpj_raw"] != "07152165000128")

# rf_fim e rf_rent limitados à ultima_dt (evita acumular dias além da última cota disponível)
rf_na_ultima = next((h for h in reversed(rf_hist) if h["dt"] <= ultima_dt), rf_hist[0])
rf_fim  = rf_na_ultima["patrimonio"]
rf_rent = rf_na_ultima["rent_acum"]
...
# Última data disponível na CVM (máximo do DT_COMPTC do CSV)
...
<header>
  ...
  <div class="gerado">&#128197; CVM: """ + ultima_cvm_fmt + """ &nbsp;&middot;&nbsp; Gerado em """ + gerado_em + """</div>
</header>
<main>
