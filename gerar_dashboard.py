...
(function(){
  ...
  if(T.resg>0) cards.push({lbl:'Resgates', val:'\u2212'+fmtBRL(T.resg), sub:MOVS.filter(m=>m.tipo==='resgate').length+' resgate(s) no periodo', cls:'neg'});
...
(function(){
  ...
  const nR = MOVS.filter(m=>m.tipo==='resgate').length;
  const nA = MOVS.filter(m=>m.tipo==='aporte').length;
  document.getElementById('lblMovsTot').textContent =
    (TOTAIS.resg>0 ? nR+' resgate(s) \u00b7 '+fmtBRL(TOTAIS.resg) : '') +
    (TOTAIS.resg>0 && TOTAIS.capt>0 ? '  \u00b7  ' : '') +
    (TOTAIS.capt>0 ? nA+' aporte(s) \u00b7 '+fmtBRL(TOTAIS.capt) : '');

  const linhas = MOVS.slice().reverse().map(m=>{
    const neg = m.tipo==='resgate';
    return `<div style="display:flex;align-items:center;gap:12px;padding:9px 0;border-bottom:1px solid #1a2035">
      <span style="width:8px;height:8px;border-radius:50%;background:${neg?'#D85A30':'#1D9E75'};flex-shrink:0"></span>
      ...
      <span style="font-size:10px;color:#8892a8;text-transform:uppercase;letter-spacing:.06em;font-family:'IBM Plex Mono',monospace">${m.tipo}</span>
      <span class="${neg?'neg':'pos'}" style="font-family:'IBM Plex Mono',monospace;font-size:13px;font-weight:600;min-width:120px;text-align:right">${neg?'\u2212':'+'}${fmtBRL(m.valor)}</span>
    </div>`;
  }).join('');

  document.getElementById('movsBox').innerHTML = linhas + `
    <div style="display:flex;justify-content:space-between;align-items:baseline;padding-top:12px;font-size:11px;color:#8892a8">
      <span>Resgates nao contam como prejuizo: saem da base de calculo da rentabilidade.</span>
      <span style="font-family:'IBM Plex Mono',monospace">Liquido: <span class="${(TOTAIS.capt-TOTAIS.resg)>=0?'pos':'neg'}" style="font-size:13px;font-weight:600">${(TOTAIS.capt-TOTAIS.resg)>=0?'+':'\u2212'}${fmtBRL(Math.abs(TOTAIS.capt-TOTAIS.resg))}</span></span>
    </div>`;
})();
