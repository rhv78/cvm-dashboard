...
def baixar_cota_base(cnpjs_raw):
    ...
    try:
        ...
        # Mostra colunas e primeiros CNPJs para diagnóstico
        print(f"  Colunas: {list(all_rows[0].keys())}")
...
for cnpj_raw in COTAS_REGRAS:
    ...
    # Usa sempre o nome definido pelo usuário
    ...
    # Usa cota de dez/2025 como base; se não disponível, usa primeira cota de 2026
    ...
    hist = []
    # Adiciona ponto inicial de 31/dez/2025 com rentabilidade 0%
    ...
    for dt in datas_todas:
        ...
        # Pula dias com cotas zero (fundo encerrado)
        if qt == 0.0: continue
        pat  = round(c * qt, 2)
...
# RF CDI começa em 31/dez/2025 com índice 1.0
...
if f_lagunna and f_everest:
    ...
    # Primeiro dia do Lagunna após incorporação (18/03)
...
cart_hist = []
# Ponto inicial 31/dez/2025 com rent 0%
...
for i, dt in enumerate(datas_todas):
    ...
    pat_fundos = 0.0
    for f in fundos:
        # Se as cotas do fundo nessa data são zero, não soma (fundo encerrado)
        ...
        if h:
            ...
            # Usa último patrimônio disponível apenas se fundo ainda ativo
...
# Patrimônio final da carteira na última data dos fundos ativos
...
# ── GRUPOS VIRTUAIS (só visualização, não entram no total) ────────────────────
def get_fundo(cnpj_raw):
    return next((f for f in fundos if f["cnpj_raw"] == cnpj_raw), None)

def consolidar_grupo(nome, cor, cnpjs):
    """Soma patrimônio diário dos fundos do grupo. Rentabilidade baseada no patrimônio inicial somado."""
...
.sub{font-size:10px;color:#4a536b;margin-top:3px;font-family:'IBM Plex Mono',monospace}
.gerado{font-size:10px;color:#4a536b;font-family:'IBM Plex Mono',monospace}
main{padding:22px 28px;max-width:1300px;margin:0 auto}
.slbl{font-family:'IBM Plex Mono',monospace;font-size:10px;font-weight:700;color:#4a536b;text-transform:uppercase;letter-spacing:.12em;margin-bottom:12px;padding-bottom:7px;border-bottom:1px solid #1a2035;margin-top:24px}
...
.card .lbl{font-size:10px;color:#4a536b;text-transform:uppercase;letter-spacing:.08em;font-family:'IBM Plex Mono',monospace;margin-bottom:6px}
.card .val{font-size:18px;font-weight:600;font-family:'IBM Plex Mono',monospace}
.card .sub{font-size:10px;color:#4a536b;margin-top:4px;font-family:'IBM Plex Mono',monospace}
.pos{color:#1D9E75} .neg{color:#D85A30}
.tab-row{display:flex;gap:8px;margin-bottom:14px;flex-wrap:wrap}
.tab{font-size:11px;padding:6px 14px;border-radius:6px;border:1px solid #1a2035;background:transparent;color:#4a536b;cursor:pointer;font-family:'IBM Plex Mono',monospace;transition:all .15s}
...
.fc-cnpj{font-size:9px;color:#4a536b;margin-top:2px;font-family:'IBM Plex Mono',monospace}
...
.fc-lbl{color:#4a536b} .fc-val{font-family:'IBM Plex Mono',monospace}
...
.ds{font-size:10px;color:#4a536b;margin-top:3px;font-family:'IBM Plex Mono',monospace}
...
.mlbl{font-size:9px;color:#4a536b;text-transform:uppercase;letter-spacing:.08em;font-family:'IBM Plex Mono',monospace}
...
(function(){
  ...
  [
    ...
    {lbl:'CDI acumulado', val:fmtPct(T.rf_rent), sub:'jan\u2013'+T.dt_fim.slice(0,7), cls:'pos'},
  ].forEach(c=>{
...
[{nome_curto:'RF CDI',color:TOTAIS.cdi_color},{nome_curto:'Carteira Total',color:TOTAIS.cart_color},...GRUPOS].forEach(f=>{
  document.getElementById('legend').innerHTML +=
    `<span class="legend-item"><span class="legend-sq" style="background:${f.color}${f.virtual?';opacity:0.7':''}"></span>${f.nome_curto}${f.virtual?' <span style="font-size:9px;color:#4a536b">(grupo)</span>':''}</span>`;
});
...
function renderMain(tab){
  ...
  mainChart = new Chart(document.getElementById('mainChart'),{
    ...
    options:{responsive:true,maintainAspectRatio:false,
      ...
      scales:{
        x:{ticks:{autoSkip:true,maxTicksLimit:10,color:'#555e7a',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}},
        y:{ticks:{color:'#555e7a',font:{size:10},callback:v=>fmtY(tab,v)},grid:{color:'rgba(255,255,255,0.04)'}},
      },
    },
  });
}
...
function setTab(tab){
  curTab=tab;
  document.querySelectorAll('#mainChart ~ * .tab, .cbox .tab').forEach(()=>{});
...
function renderGrupoDet(g) {
  ...
  detChart = new Chart(document.getElementById('detChart'),{
    ...
    options:{responsive:true,maintainAspectRatio:false,
      ...
      scales:{
        x:{ticks:{autoSkip:true,maxTicksLimit:10,color:'#555e7a',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}},
        y:{ticks:{color:'#555e7a',font:{size:10}},grid:{color:'rgba(255,255,255,0.04)'}},
      },
    },
  });
  const pc=g.rent>=0?'pos':'neg';
...
(function(){
  ...
  // Clique mostra gráfico da carteira no detalhe
  card.onclick = () => {
    document.querySelectorAll('.fc').forEach(c=>c.classList.remove('active'));
    const box = document.getElementById('detailBox');
...
// ── PIZZA ─────────────────────────────────────────────────────────────────────
(function(){
  // Exclui EVEREST (encerrado) da composição final
  const dadosAtivos = DADOS.filter(f => f.cnpj_raw !== '07152165000128');
  const ultimaDt = TOTAIS.dt_fim;

  // Patrimônio de cada fundo na ultima_dt (menor data comum)
  ...
  document.getElementById('pieInfo').innerHTML = vals.map((v,i)=>{
    ...
    return `<div class="pie-row">
      ...
      <div>
        <div style="font-size:11px;color:#4a536b">${labels[i]}</div>
        <div style="font-size:13px;font-weight:600">${fmtBRL(v)} <span style="font-size:10px;color:#4a536b">(${pct}%)</span></div>
    ...
    <div style="border-top:1px solid #1a2035;margin-top:8px;padding-top:10px;display:flex;align-items:center;gap:10px">
      ...
      <div>
        <div style="font-size:11px;color:#4a536b">Total da Carteira</div>
        <div style="font-size:15px;font-weight:700;color:#dde2f0">${fmtBRL(total)}</div>
        <div style="font-size:10px;color:#4a536b;font-family:'IBM Plex Mono',monospace">em ${ultimaDt}</div>
      </div>
    </div>
    <div style="border-top:1px solid #1a2035;margin-top:8px;padding-top:8px">
      <div style="font-size:10px;color:#4a536b;text-transform:uppercase;letter-spacing:.08em;margin-bottom:8px;font-family:'IBM Plex Mono',monospace">Grupos · apenas visualização</div>
      ${GRUPOS.map(g=>{
        const pc=g.rent>=0?'pos':'neg';
        return `<div class="pie-row" style="margin-bottom:6px">
          ...
          <div>
            <div style="font-size:11px;color:#4a536b">${g.nome}</div>
...
saida = os.path.join(_BASE, "output", "index.html")
